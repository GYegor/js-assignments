...
function getPolynom(...args) {
    return function (x) {
        return args.length == 3 ? Math.pow(x, 2) * args[0] + x * args[1] + args[2] : args.length == 2 ? x * args[0] + args[1] : args.length == 1 ? args[0] : null;
    };
}
...
function memoize(func) {
    let cache;
    return (...args) => !cache ? cache = func(...args) : cache;
}
